...
//     const [traineeId, setTraineeId] = useState(propTraineeId || null);
//     const [agreed, setAgreed] = useState(false);
//     const [showPage, setShowPage] = useState(false);

//     // 🔹 جلب traineeId من الرابط إذا لم يُمرّر كـ prop
//     useEffect(() => {
//         const idFromUrl = searchParams.get("traineeId");
//         if (idFromUrl) setTraineeId(idFromUrl);
//     }, [searchParams]);
...
//             const res = await fetch(`/api/trainee/${traineeId}`);
//             const data = await res.json();

//             if (!res.ok || !data) {
//                 router.push("/"); // 🔸 إذا لم يوجد المستخدم
//                 return;
//             }

//             if (data.hasSeenInstructions) {
//                 // ✅ المتدرب شاهد التعليمات → تخطَّ الصفحة
//                 if (onNext) onNext();
//                 else router.push("/questions");
//             } else {
//                 // 👀 أول مرة → اعرض التعليمات
//                 setShowPage(true);
//             }
//         };
//         fetchTrainee();
//     }, [traineeId, router, onNext]);

//     const handleNext = async () => {
//         if (!traineeId) return;

//         await fetch(`/api/trainee/${traineeId}`, {
//             method: "PATCH",
//             headers: { "Content-Type": "application/json" },
//             body: JSON.stringify({ hasSeenInstructions: true }),
//         });
...
//                 className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 shadow-xl rounded-3xl p-8 sm:p-10 w-full max-w-lg"
//             >
...
export default function InstructionsClient({ traineeId: propTraineeId, onNext }) {
    ...
    // 🔹 نتحقق فقط من وجود المتدرب، لكن لا نتخطى الصفحة
    useEffect(() => {
        const fetchTrainee = async () => {
            if (!traineeId) return;
    ...
    const handleNext = async () => {
        if (onNext) onNext();
        else router.push("/questions"); // 🔸 الانتقال للأسئلة بدون أي تعديل على DB
    };
